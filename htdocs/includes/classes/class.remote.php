	...
	class ISC_REMOTE extends ISC_XML_PARSER
	{
		...
		private function checkDefaultShippingMethod(){
			...
			foreach (array_keys($fields) as $fieldId) {
				if (!$success && isc_strtolower($fields[$fieldId]->record['formfieldlabel']) == isc_strtolower(GetLang('DefaultShippingMethodLabel'))) {
		...
		public function GetVariationOptions($customerGroupId = null)
		{
			...
			if(count($optionIdsArray) == $product['vnumoptions']) {
				...
				if(GetConfig('isIntelisis')){
					...
					$newprice = applyListaPreciosEsp($productClassArray, 1, $combination['combinationid']);
					
					if($newprice != '')
					{
						$productClassArray['prodcalculatedprice'] = $newprice;
					}
