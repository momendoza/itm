...
class Diamondrings_logo extends LogoMaker
{
	/**
	 * TextFieldCount
	...
	*/
	...
	public function GenerateLogo()
	{
		...
		$this->AddImage(dirname(__FILE__) . '/logo.gif', 10, 15);
		...
		$t_box = $this->TextBox($this->Text[0], 'blackfort.otf', 'ffffff', $textSize, 0, 0);

		// determine the y position for the text
		$y_pos = 6+(($imageHeight - $t_box['height'])/2);

		if(strlen($this->Text[0]) > 0) {
			// AddText() - text, font, fontcolor, fontSize (pt), x, y, center on this width
			$text_position = $this->AddText($this->Text[0], 'blackfort.otf', 'ffffff', $textSize, $textLeft, $y_pos);
		}
